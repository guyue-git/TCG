"""处理器：CP001-9080 下单确认书（第一期唯一业务）.

输入（monitor 拼接的文本）：
    message.content         = "//15.3"
    message.quoted_content  = "#下单 9080 300017 网宿科技 限价 买入 200W"
    （quoted_content 可能带 quote_marker 叠加前缀 "##下单 ..."，容忍）

产出：output/TradeConfirm(ITC9080)_{cp}_{YYYYMMDD}_{NNN}.pdf

复用 tc_generator 包的引擎（解析/MasterFile/日历/组装/渲染），
本文件只做「消息 → 引擎调用」的适配，不含业务规则。
"""
...
def handle(message: IncomingMessage, ctx: HandlerContext) -> Path | None:
    ...
    try:
        ...
        LOG.info("[cp001_9080_order] 超出一期范围，仅记录: %s", exc)
        return None
    ...
    LOG.info("[cp001_9080_order] 已生成确认书: %s（群=%s, 编号=%s）",
